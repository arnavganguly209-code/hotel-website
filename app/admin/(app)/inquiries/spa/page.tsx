...
export default function AdminSpaInquiriesPage() {
  const [items, setItems] = useState<SpaInquiry[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (q) params.set("q", q);
      const res = await fetch(`/api/admin/inquiries/spa?${params}`, { cache: "no-store" });
      const data = await res.json();
      if (!res.ok || !data.success) throw new Error(data.error || "Failed to load");
      setItems(data.inquiries ?? []);
      setNotesDraft(
        Object.fromEntries((data.inquiries ?? []).map((i: SpaInquiry) => [i.id, i.adminNotes || ""]))
      );
    } catch (err) {
  ...
  async function update(id: number, patch: Record<string, unknown>) {
    const res = await fetch("/api/admin/inquiries/spa", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id, ...patch }),
    });
    if (res.ok) {
      setItems((prev) => prev.map((i) => (i.id === id ? { ...i, ...patch } as SpaInquiry : i)));
    }
  }

  async function remove(id: number) {
    if (!confirm("Delete this inquiry?")) return;
    const res = await fetch(`/api/admin/inquiries/spa?id=${id}`, { method: "DELETE" });
  ...
  return (
    <div className="space-y-8">
      <div>
        <p className="text-[11px] uppercase tracking-[0.25em] text-[#c5a059]">Wellness</p>
        <h1 className="mt-1 font-serif text-3xl font-light text-[#0f2420]">Spa Inquiries</h1>
        <p className="mt-2 text-sm text-[#5a635c]">Spa appointment requests submitted from the spa page.</p>
      </div>
      ...
      <div className="flex flex-wrap items-end gap-3">
        ...
        <div>
          ...
          <input
            ...
            placeholder="Name, email, treatment…"
            className="rounded-lg border border-[#c5a059]/35 bg-white px-3 py-2.5 text-sm outline-none"
          />
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {items.map((item) => (
            ...
            >
              ...
              <div className="mt-4 flex flex-wrap items-center gap-3">
                ...
                <button
                  type="button"
                  onClick={() => void update(item.id, { adminNotes: notesDraft[item.id] ?? "" })}
          ...
          {items.length === 0 ? <p className="text-sm text-[#5a635c]">No spa inquiries found.</p> : null}
        </div>
      )}
    </div>
  );
