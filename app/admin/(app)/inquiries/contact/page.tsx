...
export default function AdminContactInquiriesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setNotesDraft(
        Object.fromEntries((data.inquiries ?? []).map((i: ContactEnquiry) => [i.id, i.adminNotes || ""]))
      );
    } catch (err) {
  ...
  async function update(id: number, patch: Record<string, unknown>) {
    ...
    if (res.ok) {
      setItems((prev) => prev.map((i) => (i.id === id ? { ...i, ...patch } as ContactEnquiry : i)));
    }
  }

  async function remove(id: number) {
    if (!confirm("Delete this enquiry?")) return;
    const res = await fetch(`/api/admin/inquiries/contact?id=${id}`, { method: "DELETE" });
  ...
  return (
    <div className="space-y-8">
      <div>
        <p className="text-[11px] uppercase tracking-[0.25em] text-[#c5a059]">Enquiries</p>
        <h1 className="mt-1 font-serif text-3xl font-light text-[#0f2420]">Contact Inquiries</h1>
        <p className="mt-2 text-sm text-[#5a635c]">General inquiries submitted from the contact page.</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-4">
            ...
            >
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-2 max-w-2xl text-sm text-[#3d5a4c]">
          ...
          {items.length === 0 ? <p className="text-sm text-[#5a635c]">No contact inquiries found.</p> : null}
        </div>
      )}
    </div>
  );
