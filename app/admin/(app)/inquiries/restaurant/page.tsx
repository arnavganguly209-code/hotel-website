...
const STATUSES = ["new", "confirmed", "pending", "completed", "cancelled", "archived"];

export default function AdminDiningInquiriesPage() {
  const [items, setItems] = useState<DiningReservation[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (q) params.set("q", q);
      const res = await fetch(`/api/admin/inquiries/dining?${params}`, { cache: "no-store" });
      const data = await res.json();
      if (!res.ok || !data.success) throw new Error(data.error || "Failed to load");
      setItems(data.reservations ?? []);
      setNotesDraft(
        Object.fromEntries((data.reservations ?? []).map((i: DiningReservation) => [i.id, i.adminNotes || ""]))
      );
    } catch (err) {
  ...
  async function update(id: number, patch: Record<string, unknown>) {
    const res = await fetch("/api/admin/inquiries/dining", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id, ...patch }),
    });
    if (res.ok) {
      setItems((prev) => prev.map((i) => (i.id === id ? { ...i, ...patch } as DiningReservation : i)));
    }
  }

  async function remove(id: number) {
    if (!confirm("Delete this reservation?")) return;
    const res = await fetch(`/api/admin/inquiries/dining?id=${id}`, { method: "DELETE" });
  ...
  return (
    <div className="space-y-8">
      <div>
        <p className="text-[11px] uppercase tracking-[0.25em] text-[#c5a059]">Dining</p>
        <h1 className="mt-1 font-serif text-3xl font-light text-[#0f2420]">Restaurant Reservations</h1>
        <p className="mt-2 text-sm text-[#5a635c]">Table reservations submitted from the dining page.</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          ...
          {items.length === 0 ? <p className="text-sm text-[#5a635c]">No reservations found.</p> : null}
        </div>
      )}
    </div>
  );
