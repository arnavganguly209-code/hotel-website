...
export async function GET(req: Request) {
  ...
  const inquiries = await db.eventInquiry.findMany({ orderBy: { createdAt: "desc" } });
  const filtered = inquiries.filter((item) => {
    if (status && status !== "all" && item.status !== status) return false;
    if (q) {
      const hay = `${item.name} ${item.email} ${item.phone} ${item.eventType} ${item.company}`.toLowerCase();
...
export async function PATCH(req: Request) {
  ...
  const body = (await req.json()) as { id?: number; status?: string; adminReply?: string };
  if (!body.id) {
...
export async function DELETE(req: Request) {
  ...
  await db.eventInquiry.delete({ where: { id } }).catch(() => null);
