...
interface ArticlesHeroProps {
  hero: SiteContent["articlesPage"]["hero"];
}

export function ArticlesHero({ hero }: ArticlesHeroProps) {
  const overlay = Math.min(0.65, Math.max(0.35, hero.overlayOpacity || 0.45));
  const isVideo = hero.media?.type === "video" && Boolean(hero.media.videoSrc);
  const imageSrc = hero.media?.imageSrc || hero.imageSrc;
    ...
    <section className="relative flex min-h-[72vh] items-end overflow-hidden md:min-h-[78vh]">
      <div className="absolute inset-0">
        <MediaBackground
          ...
          imageSrc={imageSrc}
          videoSrc={isVideo ? hero.media.videoSrc : undefined}
        ...
        />
        <div
          className="absolute inset-x-0 bottom-0 h-40 bg-gradient-to-t from-[#F9F6EF] to-transparent opacity-90"
          aria-hidden
        />
      </div>
      ...
      <div className="relative z-10 w-full px-6 pb-24 pt-36 text-center lg:px-10 lg:pb-28">
        ...
        >
          ...
          <span className="text-[#D4AF37]">{hero.breadcrumbCurrent || "Articles"}</span>
        </motion.nav>
        ...
        <motion.p
          ...
          className="font-display text-sm uppercase tracking-[0.42em] text-[#D4AF37] md:text-base"
        >
          {hero.title}
        </motion.p>
